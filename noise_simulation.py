...
# Same curated 10 nt sequence used across the rest of the project (see
# test_sequences.py) -- the original "GGUGCCGAAC" turned out to have a
# fully-unpaired real ViennaRNA MFE structure (it doesn't actually fold),
# so it's replaced here with a sequence ViennaRNA confirms folds, for
# consistency with qaoa_rna_solver.py / cvar_vqe_rna_solver.py.
...
print(f"Sequence: {sequence}  ({n_qubits} qubits)")
...
for level, p in noise_levels.items():
    ...
    # Maximally-mixed-state expectation of a traceless Ising Hamiltonian is
    # 0, so quality = exp_val / ideal_expectation is ~1.0 at p=0 and decays
    # toward 0.0 as noise dominates.
    quality = exp_val / ideal_expectation if ideal_expectation else 1.0
    loss = (1 - quality) * 100

    results.append([level, p, round(exp_val, 4), round(quality, 4), round(loss, 2)])
